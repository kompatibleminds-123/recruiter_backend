...
function sanitizeSharedExportPresetSettings(raw) {
  ...
  return {
    ...
    whatsappTemplate: String(source.whatsappTemplate || source.whatsapp_template || "").replace(/\r\n/g, "\n"),
    emailTemplate: String(source.emailTemplate || source.email_template || "").replace(/\r\n/g, "\n"),
    bulkMailSubjectTemplate: String(source.bulkMailSubjectTemplate || source.bulk_mail_subject_template || "").trim(),
    bulkMailBodyTemplate: String(source.bulkMailBodyTemplate || source.bulk_mail_body_template || "").replace(/\r\n/g, "\n"),
    clientShareIntroTemplate: String(source.clientShareIntroTemplate || source.client_share_intro_template || "").replace(/\r\n/g, "\n"),
    clientShareThreadIntroTemplate: String(source.clientShareThreadIntroTemplate || source.client_share_thread_intro_template || "").replace(/\r\n/g, "\n"),
    clientShareSubjectTemplate: String(source.clientShareSubjectTemplate || source.client_share_subject_template || "").trim(),
    clientShareSignatureText: String(source.clientShareSignatureText || source.client_share_signature_text || "").replace(/\r\n/g, "\n"),
...
function sanitizeAssessment(item) {
  ...
  return {
    ...
    recruiterNotes: item.recruiterNotes ?? item.recruiter_notes ?? item.recruiterContextNotes ?? item.recruiter_context_notes ?? p.recruiterNotes ?? p.recruiter_notes ?? p.recruiterContextNotes ?? p.recruiter_context_notes ?? "",
    recruiterContextNotes: item.recruiterContextNotes ?? item.recruiter_context_notes ?? item.recruiterNotes ?? item.recruiter_notes ?? p.recruiterContextNotes ?? p.recruiter_context_notes ?? p.recruiterNotes ?? p.recruiter_notes ?? "",
    otherPointers: item.otherPointers ?? item.other_pointers ?? p.otherPointers ?? p.other_pointers ?? "",
